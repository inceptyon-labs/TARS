...
export function AgentsPage() {
  ...
  const agents = useMemo(() => {
    ...
    if (profilesInventory?.agents) {
      allAgents.push(...profilesInventory.agents);
    }

    return allAgents.filter((agent) => {
      if (agent.scope.type === 'Plugin' && isProfileMarketplacePath(agent.path)) {
        return false;
      }
      return true;
    });
  ...
  async function handleCreateAgent() {
    ...
    setCreating(true);
    try {
      const toolName = newAgentName.trim();
          ...
          <div className="flex-1 overflow-auto p-3">
            {isLoading ? (
              ...
              <>
                ...
                {renderAgentGroup('Plugin Agents', groupedAgents.plugin, true)}
        ...
        <div className="flex-1 overflow-hidden bg-background">
          {loadingAgent ? (
            ...
            <MarkdownEditor
              ...
              readOnly={!isAgentEditable(selectedAgent.scope, selectedAgent.path)}
            />
          ) : (
            <div className="flex flex-col items-center justify-center h-full gap-4">
