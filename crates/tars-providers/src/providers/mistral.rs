//! Mistral provider.
//!
//! Simple-storage provider: auth-check only via `GET /v1/models` with
//! `Authorization: Bearer`. No model list surfaced, no balance.
...
impl Provider for MistralProvider {
    ...
    fn metadata(&self) -> &'static ProviderMetadata {
        metadata_for(ProviderId::Mistral)
    }
    ...
    async fn validate_key(&self, key: &str) -> Result<ValidationResult, ProviderError> {
        ...
        match status {
            ...
            StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => Ok(ValidationResult {
                valid: false,
                message: Some(format!(
                    "Key rejected by Mistral (HTTP {})",
                    status.as_u16()
                )),
            }),
            other => Err(ProviderError::Http(format!("Mistral returned {other}"))),
        }
    }
...
mod tests {
    ...
    fn provider(server: &MockServer) -> MistralProvider {
        MistralProvider::with_base_url(server.uri())
    }
    ...
    async fn validate_key_valid_on_200() {
        ...
        let r = provider(&server).validate_key("good").await.unwrap();
        assert!(r.valid);
    }
    ...
    fn metadata_matches_registry() {
        ...
        assert_eq!(p.id(), ProviderId::Mistral);
        assert_eq!(p.metadata().display_name, "Mistral");
