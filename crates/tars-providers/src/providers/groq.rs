//! Groq provider.
//!
//! Simple-storage provider: auth-check only via `GET /openai/v1/models`
//! (OpenAI-compatible endpoint) with `Authorization: Bearer`. No model list
//! surfaced, no balance.
...
impl Provider for GroqProvider {
    ...
    fn metadata(&self) -> &'static ProviderMetadata {
        metadata_for(ProviderId::Groq)
    }

    async fn validate_key(&self, key: &str) -> Result<ValidationResult, ProviderError> {
        let url = format!("{}/openai/v1/models", self.base_url);
        let resp = self
            .client
            .get(&url)
            .bearer_auth(key)
        ...
        match status {
            ...
            StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => Ok(ValidationResult {
                valid: false,
                message: Some(format!("Key rejected by Groq (HTTP {})", status.as_u16())),
            }),
            other => Err(ProviderError::Http(format!("Groq returned {other}"))),
        }
    }
...
mod tests {
    ...
    fn provider(server: &MockServer) -> GroqProvider {
        GroqProvider::with_base_url(server.uri())
    }
    ...
    async fn validate_key_valid_on_200() {
        ...
        Mock::given(method("GET"))
            ...
            .and(header("authorization", "Bearer gsk_good"))
        ...
        let r = provider(&server).validate_key("gsk_good").await.unwrap();
        assert!(r.valid);
    }
    ...
    async fn validate_key_invalid_on_401() {
        ...
        let r = provider(&server).validate_key("gsk_bad").await.unwrap();
        assert!(!r.valid);
    ...
    fn metadata_matches_registry() {
        ...
        assert_eq!(p.metadata().display_name, "Groq");
